#!/usr/bin/env python3
"""
Скрипт для возврата длительности импульса PPS на 100 мс

Использование:
    sudo python3 set_pps_100ms.py

Заменяет значение 5 мс (5000000 нс) на 100 мс (100000000 нс) в бинарнике shiwatime
"""
...
OLD_VALUE_NS = 5000000    # 0x004C4B40 (5 мс)
NEW_VALUE_NS = 100000000  # 0x05F5E100 (100 мс)
...
def find_and_replace_in_binary(filepath, old_value, new_value):
    ...
    if replacements == 0:
        ...
        if data.find(new_le_check) != -1:
            print(f"✓ Значение {new_value:,} (100 мс) уже присутствует в бинарнике")
...
def main():
    print("=" * 80)
    print("Возврат длительности импульса PPS на 100 мс")
    print("=" * 80)
    print()
    ...
    if os.path.exists(BACKUP_PATH):
        print(f"ℹ Резервная копия найдена: {BACKUP_PATH}")
        print("  (Можно использовать для восстановления)")
    else:
        print(f"⚠ Резервная копия не найдена: {BACKUP_PATH}")
        print("  (Рекомендуется создать резервную копию перед изменениями)")
    ...
    print(f"Поиск значения {OLD_VALUE_NS:,} (5 мс) в бинарнике...")
    print()
    ...
    if find_and_replace_in_binary(BINARY_PATH, OLD_VALUE_NS, NEW_VALUE_NS):
        print()
        print("=" * 80)
        print("✓ Возврат длительности импульса на 100 мс выполнен успешно!")
        print("=" * 80)
        print()
        ...
        print("  - Значение 5 мс не найдено в бинарнике")
        print("  - Значение 100 мс уже установлено")
        print("  - Значение закодировано в другом формате")
        print("=" * 80)
        print()
        print("Альтернативный способ восстановления из резервной копии:")
        if os.path.exists(BACKUP_PATH):
